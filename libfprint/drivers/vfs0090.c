...
static int usb_error_to_fprint_fail(struct fp_img_dev *idev, int status)
{
	...
	switch (fpi_imgdev_get_action_state(idev)) {
			       ...
			       async_operation_cb callback, void* callback_data)
{
	...
	libusb_fill_bulk_transfer(vdev->transfer, fpi_imgdev_get_usb_dev(idev),
				...
				async_operation_cb callback, void* callback_data)
{
	...
	case VFS_READ_BULK:
		libusb_fill_bulk_transfer(vdev->transfer,
		                          fpi_imgdev_get_usb_dev(idev), EP_IN,
					  buffer, buffer_size,
					  async_read_callback, op_data,
					  VFS_USB_TIMEOUT);
		break;
...
{
	struct tls_handshake_t *tlshd = fpi_ssm_get_user_data(ssm);
	struct fp_img_dev *idev = IMG_DEV_FROM_SSM(ssm);
	struct vfs_dev_t *vdev = VFS_DEV_FROM_IMG(idev);
	struct vfs_init_t *vinit = fpi_ssm_get_user_data(fpi_ssm_get_parentsm(ssm));
...
{
	struct tls_handshake_t *tlshd = fpi_ssm_get_user_data(ssm);
	struct fpi_ssm *parent_ssm = fpi_ssm_get_parentsm(ssm);
	struct fp_img_dev *idev = IMG_DEV_FROM_SSM(ssm);

	if (fpi_ssm_get_error(ssm)) {
		fpi_imgdev_session_error(idev, fpi_ssm_get_error(ssm));
		fpi_ssm_mark_aborted(parent_ssm, fpi_ssm_get_error(ssm));
	} else {
...
static void start_handshake_ssm(struct fp_img_dev *idev, struct fpi_ssm *parent_ssm)
{
	...
	ssm = fpi_ssm_new(fpi_imgdev_get_dev(idev), handshake_ssm, TLS_HANDSHAKE_STATE_LAST);
	fpi_ssm_set_user_data(ssm, g_new0(struct tls_handshake_t, 1));
	fpi_ssm_set_subsm(ssm, parent_ssm);
...
{
	struct vfs_init_t *vinit = fpi_ssm_get_user_data(ssm);
	struct fp_img_dev *idev = IMG_DEV_FROM_SSM(ssm);
	struct vfs_dev_t *vdev = VFS_DEV_FROM_IMG(idev);
	...
	case INIT_STATE_SEQ_6:
		send_init_sequence(ssm, fpi_ssm_get_cur_state(ssm) - INIT_STATE_SEQ_1);
		break;
...
static int dev_open(struct fp_img_dev *idev, unsigned long driver_data)
{
	...
	vdev->buffer_length = 0;
        ...
        udev = fpi_imgdev_get_usb_dev(idev);
	...
	ssm = fpi_ssm_new(fpi_imgdev_get_dev(idev), init_ssm, INIT_STATE_LAST);
	fpi_ssm_set_user_data(ssm, g_new0(struct vfs_init_t, 1));
...
static void finger_image_download_callback(struct fpi_ssm *ssm)
{
	...
	if (!fpi_ssm_get_error(ssm)) {
		fpi_ssm_mark_completed(fpi_ssm_get_parentsm(ssm));
	} else {
		fp_err("Scan failed failed at state %d, unexpected"
		       "device reply during image download", fpi_ssm_get_cur_state(ssm));
		fpi_imgdev_session_error(idev, fpi_ssm_get_error(ssm));
		fpi_ssm_mark_aborted(fpi_ssm_get_parentsm(ssm), fpi_ssm_get_error(ssm));
	}
...
static void start_finger_image_download_subsm(struct fpi_ssm *parent_ssm)
{
	...
	ssm = fpi_ssm_new(fpi_imgdev_get_dev(idev),
			  finger_image_download_ssm,
			  IMAGE_DOWNLOAD_STATE_LAST);

	fpi_ssm_set_user_data(ssm, g_new0(struct image_download_t, 1));
	fpi_ssm_set_subsm(ssm, parent_ssm);
...
static void start_finger_scan(struct fp_img_dev *idev)
{
	...
	ssm = fpi_ssm_new(fpi_imgdev_get_dev(idev), finger_scan_ssm, SCAN_STATE_LAST);
...
static void activate_device_interrupt_callback(struct fp_img_dev *idev, int status, void *data)
{
	...
	if (status == LIBUSB_TRANSFER_COMPLETED) {
		...
		if (interrupt_type == VFS_SCAN_WAITING_FOR_FINGER) {
			struct fp_dev *dev = fpi_imgdev_get_dev(idev);
			if (fpi_imgdev_get_action(idev) == IMG_ACTION_ENROLL &&
			    dev->state == DEV_STATE_ENROLLING) {
				struct fpi_ssm *child_ssm;
				child_ssm = fpi_ssm_new(fpi_imgdev_get_dev(idev),
							finger_scan_ssm,
							SCAN_STATE_LAST);
...
static void activate_ssm(struct fpi_ssm *ssm)
{
	...
	case ACTIVATE_STATE_SCAN_MATRIX:
		send_activate_sequence(ssm, fpi_ssm_get_cur_state(ssm) - ACTIVATE_STATE_SEQ_1);
		break;
...
static int dev_activate(struct fp_img_dev *idev, enum fp_imgdev_state state)
{
	...
	ssm = fpi_ssm_new(fpi_imgdev_get_dev(idev), activate_ssm, ACTIVATE_STATE_LAST);
...
static void deactivate_ssm(struct fpi_ssm *ssm)
{
	...
	case DEACTIVATE_STATE_SEQ_2:
		send_deactivate_sequence(ssm, fpi_ssm_get_cur_state(ssm) - DEACTIVATE_STATE_SEQ_1);
		break;
...
static void dev_deactivate(struct fp_img_dev *idev)
{
	...
	ssm = fpi_ssm_new(fpi_imgdev_get_dev(idev), deactivate_ssm, DEACTIVATE_STATE_LAST);
...
{
	struct vfs_dev_t *vdev = fpi_imgdev_get_user_data(idev);
	libusb_device_handle *udev = fpi_imgdev_get_usb_dev(idev);
