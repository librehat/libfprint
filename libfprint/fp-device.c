...
fp_device_task_return_in_idle_cb (gpointer user_data)
{
  ...
  g_assert (priv->current_action == data->action);

  g_debug ("Idle completing action %d!", priv->current_action);
  ...
  else
    {
      switch (data->type) {
        case FP_DEVICE_TASK_RETURN_INT:
          g_task_return_int (task, GPOINTER_TO_INT (data->data));
          break;
        case FP_DEVICE_TASK_RETURN_BOOL:
          g_task_return_boolean (task, GPOINTER_TO_UINT (data->data));
          break;
        case FP_DEVICE_TASK_RETURN_OBJECT:
                               ...
                               gpointer                return_data)
{
  ...
  data->action = priv->current_action;
