...
static void
on_list_completed (FpDevice *dev, GAsyncResult *res, void *user_data)
{
  ListData *list_data = user_data;
  ...
  if (!error)
    {
      guint i;
      char buf[128];
      g_autolist(FpPrint) to_delete = NULL;

      g_debug ("Device contains %u prints\n", prints->len);
          ...
          if (idx > 0 && idx <= prints->len)
            {
              ...
              to_delete = g_list_prepend (to_delete, g_object_ref (print));
            }
          else if (strncmp (buf, "A", G_N_ELEMENTS (buf)) == 0)
            {
              for (i = 0; i < prints->len; ++i)
                {
                  FpPrint *print = prints->pdata[i];
          ...
          if (to_delete)
            {
              ...
              for (l = to_delete; l; l = l->next)
                {
                  FpPrint *print;
                  fp_device_delete_print (dev, );

                }
            }

          for (i = 0; i < prints->len; ++i)
            g_object_unref (prints->pdata[i]);
        }

    }
  else
    {
      g_warning ("Getting prints failed with error %s\n", error->message);
    }
}
...
main (void)
{
  ...
  printf ("This program will report the prints saved in device\n");
