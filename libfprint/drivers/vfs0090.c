...
static gboolean cipher_operation(int error, struct fp_img_dev *idev)
{
	if (error != 0) {
		fp_err("Cypher operation failed: %d", error);
		if (idev) {
			fpi_imgdev_session_error(idev, -EIO);
		}
		return FALSE;
...
static int dev_activate(struct fp_img_dev *idev, enum fp_imgdev_state state)
{
	...
	g_print("DEV ACTIVATE with state %d\n",state);

        // switch (state) {
        // case IMG_ACQUIRE_STATE_ACTIVATING:
 ...
 //                while (TRUE) {
 //                	int finger_state = wait_for_finger_state(idev);

 //                	switch (finger_state) {
 //                        case VFS_SCAN_FINGER_ON_SENSOR:
	...
	case IMGDEV_STATE_AWAIT_FINGER_ON:
	        ...
	        while (TRUE) {
                	...
                	if (finger_state == VFS_SCAN_FINGER_ON_SENSOR) {
                        ...
                        //     	fpi_imgdev_report_finger_status(idev, FALSE);
                        // 	fpi_imgdev_abort_scan(idev, -EIO);
                        // }
                 ...
                 //                fpi_imgdev_report_finger_status(idev, TRUE);
                 //                break;
                 //        case VFS_SCAN_FAILED_TOO_FAST:
                 //        case VFS_SCAN_FAILED_TOO_SHORT:
                 //                fpi_imgdev_abort_scan(idev, -EIO);
                 //                break;
                	// }
                }
