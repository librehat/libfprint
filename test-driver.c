...
static void async_write_callback(struct libusb_transfer *transfer)
{
	...
	struct fp_img_dev *idev = fpi_ssm_get_data(ssm);
...
void
next_timeout_cb(struct fp_dev *dev,
			      void          *data)
{
  fpi_ssm_next_state_timeout_cb(dev, data);
}
...
static void activate_ssm(fpi_ssm *ssm, struct fp_dev *_dev, void *user_data)
{
	...
	case SSM_SUBMIT_IMAGE:
		...
		fpi_timeout_add(VFS_SSM_TIMEOUT, next_timeout_cb, _dev, ssm);
		break;
...
struct fp_img_driver *fp_shared_driver_img = &vfs0050_driver;
